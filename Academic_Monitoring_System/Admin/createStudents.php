...
if (isset($_POST['save'])) {
    ...
    $selectedSessionId = $_POST['sessionId'];

    // Get the current year
    $currentYear = date("Y");

    // Retrieve the last inserted ID from tblstudents
    $query = mysqli_query($conn, "SELECT id FROM tblstudents ORDER BY id DESC LIMIT 1");
    $lastInsertedId = 0;
    ...
    // Generate the admission number by combining the year and ID
    $admissionNumber = $currentYear . sprintf('%04d', $newId); // Padded to 4 digits

    $query = mysqli_query($conn, "INSERT INTO tblstudents(firstName, lastName, otherName, admissionNumber, password, classId, classArmId, dateCreated, session) 
    VALUES ('$firstName', '$lastName', '$otherName', '$admissionNumber', '12345', '$classId', '$classArmId', '$dateCreated', '$selectedSessionId')");
...
if (isset($_POST['update'])) {
    ...
    $classArmId = $_POST['classArmId'];
    $dateCreated = date("Y-m-d");
    $selectedSessionId = $_POST['sessionId']; // Dagdag na variable para sa napiling session

    $query = mysqli_query($conn, "UPDATE tblstudents SET firstName='$firstName', lastName='$lastName',
        otherName='$otherName', admissionNumber='$admissionNumber', password='12345', classId='$classId', classArmId='$classArmId', session='$selectedSessionId'
        WHERE Id='$Id'");
                      ...
                      <input type="text" class="form-control" required name="admissionNumber" value="<?php echo $row['admissionNumber'];?>" id="exampleInputFirstName" >
                         ...
                         <div class="col-xl-6">
                            <label class="form-control-label">School Year<span class="text-danger ml-2">*</span></label>
                              <?php
                         $qry = "SELECT DISTINCT sessionName, Id FROM tblsessionterm";
                          $result = $conn->query($qry);
                          $num = $result->num_rows;

                          if ($num > 0) {
                            echo '<select required name="sessionId" class="form-control mb-3">';
                            echo '<option value="">--Select School Year--</option>';
                            
                            $sessionNames = array();
                            
                            while ($rows = $result->fetch_assoc()) {
                              $sessionName = $rows['sessionName'];
                              if (!in_array($sessionName, $sessionNames)) {
                                $sessionNames[] = $sessionName;
                                
                                
                                $selected = '';
                                if (isset($_POST['sessionId']) && $_POST['sessionId'] == $rows['Id']) {
                                  $selected = 'selected';
                                }
                                
                                echo '<option value="'.$rows['Id'].'" '.$selected.'>'.$sessionName.'</option>';
                              }
                            }
