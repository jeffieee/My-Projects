...
if (isset($_POST['save'])) {
    ...
    // Verify old password
    $selectQuery = mysqli_query($conn, "SELECT password, isChange FROM tblstudents WHERE admissionNumber = '$admissionNumber'");
    ...
    if ($oldPass == $decryptedOldPass || $oldPass == '12345') {
        // Old password is correct or matches the default password, proceed with the update

        // Encrypt the new password
        $encryptedNewPass = encryptData($newPass);

        // Update password and set isChange to 1
        $updateQuery = mysqli_query($conn, "UPDATE tblstudents SET password = '$encryptedNewPass', isChange = 1 WHERE admissionNumber = '$admissionNumber'");
                                    ...
                                    <form method="post" onsubmit="return confirmPasswordChange()">
                                        <div class="form-group row mb-3">
                                            <div class="col-xl-6">
                                                ...
                                                <input type="text" class="form-control" name="admissionNumber" id="exampleAdmissionNumber" placeholder="Admission Number">
                                            </div>
                                            <div class="col-xl-6">
